...
exports.generateContent = (users) => {
    let content = "";
    let count = 1;
    const rows = users.map(({login, username, createdAt}) => 
        `<tr>
            <th>${count++}</th>
            <th>${login}</th>
            <th>${username}</th>
...
exports.generateFileName = () =>{
    ...
    return `users-${random(100000, 500000)}.html`;
};
...
exports.sendFile = async(ctx, filename, filePath) =>{
    return new Promise((resolve, reject) => {
        ...
        stream.on('close',()=> stat(filePath, (err) => err ? reject(error) : resolve()));
